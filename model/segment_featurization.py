...
# ------------------------------
# Avg. walking speed ~5km/h (https://en.wikipedia.org/wiki/Walking)
AVG_WALK_SPEED = 1.388
# Avg. bike speed ~15.5 km/h (https://en.wikipedia.org/wiki/Bicycle_performance)
AVG_BIKE_SPEED = 4.305556
# Avg. car speed speed ~12.1 km/h in Beijing (http://qz.com/163178/a-big-reason-beijing-is-polluted-the-average-car-goes-7-5-miles-per-hour/)
AVG_CAR_SPEED = 3.361111
# Avg. bus speed ~19 km/h in Beijing (http://www.chinabrt.org/en/cities/beijing.aspx)
...
# Change Velocity Rate Threshold: x-times from reference velocity
THRESHOLD_CHANGE_VEL_RATE = 5
...
class SegmentFeaturization(object):
    ...
    def __init__(self, segment_file_path):
        ...
        self.df_seg = pd.read_csv(segment_file_path, sep=",", low_memory=False)
    ...
    def featurize_segment(self):
        ...
        seg_columns = [SEG_ID_STR,                  # Segment ID,
                       # time difference between start and end timestamps of the segment
                       TIME_TOTAL_STR,
                       DISTANCE_TOTAL_STR,          # Sum of all Distances in the segment
                       VEL_MEAN_DISTANCE_STR,       # Total Distance/Delta Time
                       # Sum of all Velocities at all points in the segment/number of segments
                       ...
                       # Num. of points in segment velocity below threshhold/Segment Distance
                       VEL_LOW_RATE_STR,
                       # Num. of points in segment where CHANGE in velocity above threshhold/Segment Distance
                       VEL_CHANGE_RATE_STR,
                       # Num. of points in segment bearing CHANGE varied from threshhold/Segment Distance
        ...
        for s_id in segment_ids:
            ...
            # print("time_total ", pd_segment.time_delta)
            ...
            # Total time
            print(pd_segment)
            time_total = pd_segment['time_delta'].sum()
            print("time_total ", time_total)

            # Total distance
            distance_total = np.sum(pd_segment.distance_delta)
            # print("distance_total ", pd_segment.distance_delta)
            ...
            vel_mean_segment = np.sum(
                pd_segment.velocity_delta)/len(pd_segment)
            ...
            # Velocity Low Rate
            vel_low_rate = pd_segment.velocity_delta[pd_segment.velocity_delta < THRESHOLD_LOW].count(
            )/distance_total

            # Velocity High Rate
            vel_change_rate = pd_segment.velocity_delta_ratio[pd_segment.velocity_delta_ratio > THRESHOLD_CHANGE_VEL_RATE].count(
            )/distance_total

            # Bearing Rate
            bearing_change_rate = pd_segment.bearing_delta_redirect[pd_segment.bearing_delta_redirect > THRESHOLD_CHANGE_BEARING_RATE].count(
            )/distance_total
            ...
            df_seg = pd.concat(
                [df_seg, df_temp], ignore_index=True)
