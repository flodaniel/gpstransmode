...
class LabelProcessing(object):
    ...
    def __init__(self):
        ...
        self.df_labels = pd.DataFrame(
            data=np.zeros((0, len(columns))), columns=columns)
        ...
        for dirpath, dirnames, filenames in os.walk(label_dir):
            ...
            for filename in [f for f in filenames if f.startswith(LABELS_STR)]:
                df_temp = pd.read_csv(os.path.join(
                    dirpath, filename), sep='\t', header=0)
                self.df_labels = pd.concat(
                    [self.df_labels, df_temp], ignore_index=True)
    ...
    def process_time_label(self):
        ...
        output: None
        """
        print("STARTING process_time_label")
        # Create a new column: Get duration in seconds of a single track

        self.df_labels[DURATION_SEC_STR] = self.df_labels.apply(
            lambda row: self.calc_delta_time(row[START_TIME_STR], row[END_TIME_STR]), axis=1)
        ...
        for i, row in self.df_labels.iterrows():
            if i == 0:
                ...
                time_between_track = self.calc_delta_time(
                    end_last_track, start_next_track)
        ...
        self.df_labels[END_TIMESTAMP_STR] = [self.dt_to_timestamp_since_epoch(
            t, YMDHMS_STR) for t in self.df_labels[END_TIME_STR]]
        self.df_labels[START_TIMESTAMP_STR] = [self.dt_to_timestamp_since_epoch(
            t, YMDHMS_STR) for t in self.df_labels[START_TIME_STR]]
    ...
    def search_trajectory_data(self, file_struct_path):
        ...
        with open(file_struct_path, 'w') as csvfile:
            ...
            fts_writer.writerow(
                [DIRECTORY_STR, FILENAME_STR, START_TIMESTAMP_STR, END_TIMESTAMP_STR])
            ...
            for directory in np.unique(self.df_labels.directory):
                ...
                for filegps in files:
                    ...
                    start = self.dt_to_timestamp_since_epoch(
                        f_gps[5][0] + " " + f_gps[6][0], YMDHMS_FORMAT2_STR)
                    end = self.dt_to_timestamp_since_epoch(
                        f_gps[5][len(f_gps)-1] + " " + f_gps[6][len(f_gps)-1], YMDHMS_FORMAT2_STR)
    ...
    def create_gps_points_master(self, file_gps_path, file_struct_path):
        ...
        with open(file_gps_path, 'w') as csvfile:
            ...
            seg_writer.writerow(
                [SEG_ID_STR, MODE_STR, LATITUDE_STR, LONGITUDE_STR, DATE_STR, TIME_STR])
            csvfile.close()
        ...
        for i, row_lbl in self.df_labels.iterrows():
            ...
            dir_searched = df_files_info[df_files_info[DIRECTORY_STR]
                                         == row_lbl[DIRECTORY_STR]]
            ...
            if len(res_file) == 0:
                ...
                in_directory = os.path.join(
                    row_lbl[DIRECTORY_STR], TRAJECTORY_STR)
                file_path = os.path.join(
                    in_directory, df_files_info.loc[idx[0], FILENAME_STR])

                # Now open the file
                f_gps = pd.read_csv(file_path, skiprows=6, header=None)
                # Get timestamp of each GPS point
                # f_gps[TIMESTAMP_GPS_STR] = map(self.dt_ymd_format2_to_timestamp_since_epoch, f_gps[5] + " " + f_gps[6])
                f_gps[TIMESTAMP_GPS_STR] = pd.to_datetime(
                    f_gps[5] + " " + f_gps[6])

                # Mark as TRUE all timesstamps, such that t_start < timestamp < t_end
                starttime_to_search_t = datetime.datetime.utcfromtimestamp(
                    starttime_to_search)
                endtime_to_search_t = datetime.datetime.utcfromtimestamp(
                    endtime_to_search)
                ...
                f_gps[VALID_GPS_MASK_STR] = (f_gps[TIMESTAMP_GPS_STR] >= starttime_to_search_t) & (
                    f_gps[TIMESTAMP_GPS_STR] <= endtime_to_search_t)
                # Loop over data
                resultdf = f_gps.loc[f_gps[VALID_GPS_MASK_STR]].copy()
                resultdf[SEG_ID_STR] = row_lbl[SID_STR]
                resultdf[MODE_STR] = row_lbl[TRANS_MODE_STR]
                print(resultdf)
                resultdf = resultdf[[
                    SEG_ID_STR, MODE_STR, 0, 1, 5, 6]]
                # resultdf = resultdf[[
                #     SEG_ID_STR, MODE_STR, LATITUDE_STR, LONGITUDE_STR, DATE_STR, TIME_STR]]
                resultdf.to_csv(file_gps_path, header=False,
                                index=False, mode='a')
