...
class GpsDataEngineering(object):
    ...
    def calculate_segment_characteristics(self, segment_file_path):
        ...
        for seg_id in segment_ids:
            ...
            print("Segment ", str(cnt), ": ", seg_id,
                  " starting at index ", first_segment_index)
            ...
            for i, row in pd_segment.iterrows():
                ...
                if t_delta == 0:
                    t_delta = 1
                    print(
                        "Delta time adjusted to 1 second sice two adjacent points with same time stamp at", row["time"])
                ...
                try:
                    ...
                    print("Div By 0 at: SegID | Timestamp | Time | DDelta |  TDelta",
                          seg_id, row[TIMESTAMP_STR], row[TIME_STR], d_delta, t_delta)
            ...
            self.df_gps[first_segment_index:first_segment_index +
                        len(pd_segment)] = pd_segment

            with open(segment_file_path, 'a') as f:
                (self.df_gps[first_segment_index:first_segment_index +
                 len(pd_segment)]).to_csv(f, header=False, index=False)
